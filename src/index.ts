...
async function toInlineDataParts(inputs: InlineImageInput[] | undefined): Promise<Array<{ inline_data: { mime_type: string; data: string } }>> {
...
mcp.tool(
  ...
  async (args) => {
    const { prompt, images, saveToFilePath } = args as { prompt: string; images: InlineImageInput[]; saveToFilePath?: string };
